...
var METADATA_SEPARATOR_CHAR = "#";
var NS_KEY_SEPARATOR_CHAR = ":";
...
function Storeit(namespace, storageProvider) {
    ...
    if (namespace.indexOf(METADATA_SEPARATOR_CHAR) !== -1) {
    ...
    function isKeyThisNamespace(key) {
        var keyNamespace = key.split(NS_KEY_SEPARATOR_CHAR)[0];
        return keyNamespace === namespace;
    }

    // Listen for changes in the storageProvider.
    function storageAdded(key, newValue) {
        if (isKeyThisNamespace(key)) {
            console.log("added", namespace, key, newValue);
            setCache(key, newValue);
        }
    }

    function storageModified(key, newValue, oldValue) {
        if (isKeyThisNamespace(key)) {
            console.log("modified", namespace, key, newValue, oldValue);
            setCache(key, newValue);
        }
    }
    ...
    function initialize() {
        ...
        if (needsInitialization) {
            ...
            if (storageProvider.on) { // Are we using a newer provider that supports events?
                storageProvider.on("added", storageAdded);
                storageProvider.on("modified", storageModified);
            }
        }
    ...
    function nskey(key, modifier) {
        return namespace + (modifier ? METADATA_SEPARATOR_CHAR + modifier : "") + NS_KEY_SEPARATOR_CHAR + key;
    }
    ...
    function removeKeyFromCacheAndPublish(key) {
    ...
    function removeKey(key) {
        if (has(key)) {
            var removedValue = removeKeyFromCacheAndPublish(key);
    ...
    that.clear = function () {
        ...
        var removeFn = options.publishRemoveOnClear ? removeKeyFromCacheAndPublish : removeKeyFromCache;
        Object.keys(cache).reverse().forEach(removeFn);
    ...
    storageProvider.isMetadataKey = Storeit.isMetadataKey;
}
...
Storeit.isMetadataKey = function (key) {
    return key.indexOf(METADATA_SEPARATOR_CHAR) !== -1 || key.indexOf(NS_KEY_SEPARATOR_CHAR) === -1;
};
